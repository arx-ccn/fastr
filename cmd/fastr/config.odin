...
Config :: struct {
	...
	sync_peers:                  []string,
	// Seconds between sync passes. FASTR_SYNC_INTERVAL. Default: 3600 (1h).
	sync_interval:               u64,
}
...
// Parse FASTR_SYNC_PEERS: comma-separated relay URLs, whitespace-trimmed,
// empties dropped. URL validity is checked per peer at startup.
@(private = "file")
parse_sync_peers :: proc(allocator := context.allocator) -> []string {
	raw, found := os.lookup_env("FASTR_SYNC_PEERS", context.temp_allocator)
	if !found || strings.trim_space(raw) == "" {
		return nil
	}
	parts := strings.split(raw, ",", allocator)
	out := make([dynamic]string, 0, len(parts), allocator)
	for part in parts {
		url := strings.trim_space(part)
		if url != "" {
			append(&out, url)
		}
	}
