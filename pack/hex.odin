...
//
// Only the scalar LUT path is implemented for now. `hex_encode` and
// `hex_decode` are dispatch wrappers so a SIMD (SSSE3-equivalent) path can be
...
hex_decode :: proc(src: []u8, dst: []u8) -> (n: int, err: Error) {
	...
	return hex_decode_scalar(src, dst)
}
...
hex_encode_into :: proc(src: []u8, buf: ^[dynamic]u8) {
	...
	hex_encode_scalar(src, buf[base:])
}

@(private)
hex_encode_scalar :: proc(src: []u8, dst: []u8) {
	for b, i in src {
		p := pair_lut[b]
		dst[i * 2] = u8(p)
		dst[i * 2 + 1] = u8(p >> 8)
...
@(private)
hex_decode_scalar :: proc(src: []u8, dst: []u8) -> (n: int, err: Error) {
	count := len(src) / 2
	for i in 0 ..< count {
