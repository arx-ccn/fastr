...
// One client connection. `pending` holds bytes received past the HTTP
// upgrade response — the first WebSocket frames can ride the same TCP
// segment as the 101 headers and must not be dropped.
Conn :: struct {
	sock:    net.TCP_Socket,
	pending: [dynamic]u8,
	poff:    int,
...
ws_connect :: proc(url: string) -> Conn {
	addr := url
	if strings.has_prefix(addr, "ws://") {
		addr = addr[5:]
	}
	...
	if dial_err != nil {
		fail("connect %s: %v", addr, dial_err)
	}
...
send_text :: proc(sock: net.TCP_Socket, payload: string) {
	send_frame(sock, 0x81, transmute([]u8)payload)
}

send_frame :: proc(sock: net.TCP_Socket, first_byte: u8, p: []u8) {
	header: [14]u8
	header[0] = first_byte
	n := 0
	switch {
...
read_exact :: proc(c: ^Conn, buf: []u8) {
	off := 0
	// Drain bytes buffered during the upgrade first.
...
// Read the next text frame (temp-allocated). Close frames abort the run,
// pings are answered with pongs (relays like rnostr heartbeat and drop
// connections that never pong); other control frames are ignored.
recv_text :: proc(c: ^Conn) -> string {
	for {
		hdr: [2]u8
		read_exact(c, hdr[:])
		opcode := hdr[0] & 0x0F
		...
		if opcode == 0x9 {
			send_frame(c.sock, 0x8A, payload)
		}
	}
}
...
ingest_worker :: proc(w: ^Worker) {
	conn := ws_connect(w.url)
	defer conn_close(&conn)
	for ev in w.events {
		t0 := time.tick_now()
		send_text(conn.sock, ev.msg)
		for {
			msg := recv_text(&conn)
...
query_worker :: proc(w: ^Worker) {
	...
	for i in 0 ..< w.queries {
		...
		send_text(conn.sock, fmt.tprintf(`["CLOSE","%s"]`, sub_id))
		free_all(context.temp_allocator)
	}
}
...
cmd_neg_sync :: proc(url: string, filter_json: string, have_count: int) {
	...
	start := time.tick_now()
	send_text(conn.sock, fmt.tprintf(`["NEG-OPEN","neg-bench",%s,"%s"]`, filter_json, hex_str(init_msg, context.temp_allocator)))
	...
	outer: for {
		...
		send_text(conn.sock, fmt.tprintf(`["NEG-MSG","neg-bench","%s"]`, hex_str(next, context.temp_allocator)))
		free_all(context.temp_allocator)
	}
	wall := time.tick_since(start)

	send_text(conn.sock, `["NEG-CLOSE","neg-bench"]`)
